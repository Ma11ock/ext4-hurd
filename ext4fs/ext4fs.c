/* Main entry point for the ext4 file system translator
   ...
   Converted for ext4fs by Miles Bader <miles@gnu.ai.mit.edu>
...
char *diskfs_server_name = "ext4fs";
char *diskfs_server_version = HURD_VERSION;
char *diskfs_extra_version = "GNU Hurd; ext4 " EXT4FS_VERSION;
...
struct ext4_group_desc *group_desc_image;
...
parse_opt (int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
	...
	{
#ifdef EXT4FS_DEBUG
	  ext4_debug_flag = !ext4_debug_flag;
#else
...
main (int argc, char **argv)
{
  ...
  if (store->size < SBLOCK_OFFS + SBLOCK_SIZE)
    ext4_panic ("device too small for superblock (%Ld bytes)", store->size);
  if (store->log2_blocks_per_page < 0)
    ext4_panic ("device block size (%zu) greater than page size (%zd)",
  ...
  err = diskfs_cached_lookup (EXT4_ROOT_INO, &diskfs_root_node);
  if (err)
    ext4_panic ("can't get root: %s", strerror (err));
  else if ((diskfs_root_node->dn_stat.st_mode & S_IFMT) == 0)
    ext4_panic ("no root node!");
