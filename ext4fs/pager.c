/* Pager for ext4fs
   ...
   Converted for ext4fs by Miles Bader <miles@gnu.org>
...
static struct ext4fs_pager_stats ext4s_pager_stats =
  { .lock = PTHREAD_SPINLOCK_INITIALIZER };

#define STAT_INC(field)							      \
do { pthread_spin_lock (&ext4s_pager_stats.lock);			      \
     ext4s_pager_stats.field++;						      \
     pthread_spin_unlock (&ext4s_pager_stats.lock); } while (0)
	    ...
	    block_t *block, pthread_rwlock_t **lock)
{
  ...
  err = ext4_getblk (node, offset >> log2_block_size, 0, block);
  if (err == EINVAL)
		      ...
		      void **buf, int *writelock)
{
  ...
  ext4_debug ("reading inode %llu page %lu[%u]",
...
pending_blocks_write (struct pending_blocks *pb)
{
  if (pb->num > 0)
	...
	<< log2_dev_blocks_per_fs_block;
      ...
      ext4_debug ("writing block %u[%ld]", pb->block, pb->num);
...
file_pager_write_page (struct node *node, vm_offset_t offset, void *buf)
{
  ...
  ext4_debug ("writing inode %d page %d[%d]", node->cache_id, offset, left);
...
disk_pager_read_page (vm_offset_t page, void **buf, int *writelock)
{
  ...
  ext4_debug ("(%lld)", offset >> log2_block_size);
...
disk_pager_write_page (vm_offset_t page, void *buf)
{
  ...
  ext4_debug ("writing disk page %lld[%zu]", offset, length);
  ...
  if (pager->type == DISK)
      ...
      if (!err)
	{
	  ...
	  while (left > 0)
	    {
	      block_t disk_block;
	      err = ext4_getblk (node, block++, 1, &disk_block);
	      if (err)
		break;
	      left -= block_size;
	    }
	...
	ext4_debug ("made page %u[%lu] in inode %d partially writable",
		    page, node->allocsize - page, node->cache_id);
      else
	ext4_debug ("made page %u[%u] in inode %d writable",
      ...
      if (err == ENOSPC)
	ext4_warning ("This filesystem is out of space.");
      else if (err)
	ext4_warning ("inode=%Ld, page=0x%lx: %s",
...
diskfs_grow (struct node *node, off_t size, struct protid *cred)
{
  ...
  if (size > node->allocsize)
      ...
      if (new_end_block > end_block)
	{
	  ...
	  ext4_debug ("growing inode %d to %lu bytes (from %lu)", node->cache_id,
		      new_size, old_size);
	  ...
	  if (dn->last_page_partially_writable
		...
		(old_page_end_block > new_end_block
		 ...
		 : old_page_end_block);
	      ...
	      ext4_debug ("extending writable page %u by %d blocks"
			  "; first new block = %u",
			  trunc_page (old_size),
		...
		{
		  block_t disk_block;
		  err = ext4_getblk (node, end_block++, 1, &disk_block);
		}
	      diskfs_end_catch_exception ();
      ...
      ext4_debug ("new size: %ld%s.", new_size,
		  dn->last_page_partially_writable
		  ? " (last page writable)": "");
      if (err)
	ext4_warning ("inode=%Ld, target=%Ld: %s",
...
disk_cache_init (void)
{
  if (block_size != vm_page_size)
    ext4_panic ("Block size %u != vm_page_size %u",
  ...
  if (hurd_ihash_create (&disk_cache_bptr, HURD_IHASH_NO_LOCP))
    ext4_panic ("Can't allocate memory for disk_pager_bptr");

  /* Allocate space for disk cache blocks' info.  */
  disk_cache_info = malloc ((sizeof *disk_cache_info) * disk_cache_blocks);
  if (!disk_cache_info)
    ext4_panic ("Cannot allocate space for disk cache info");
...
disk_cache_return_unused (void)
{
  ...
  if (pending_end >= 0)
    ...
    {
      ext4_debug ("ext4fs: disk cache is starving\n");
...
retry_ref:
  ...
  if (bptr)
      ...
      if (disk_cache_info[index].flags & DC_UNTOUCHED)
	{
      ...
      ext4_debug ("cached %u -> %d (ref_count = %hu, flags = %#hx, ptr = %p)",
		  disk_cache_info[index].block, index,
		  disk_cache_info[index].ref_count,
  ...
  if (info == NULL)
    /* No place is found.  Try to release some blocks and try
       again.  */
    {
      ext4_debug ("flush %u -> %d", disk_cache_info[index].block, index);
  ...
  ext4_debug ("map %u -> %d (%p)", block, index, bptr);
  ...
  if (hurd_ihash_locp_add (disk_cache_bptr, slot, block, bptr))
    ext4_panic ("Couldn't hurd_ihash_locp_add new disk block");
...
disk_cache_block_ref_ptr (void *ptr)
{
  ...
  ext4_debug ("(%p) (ref_count = %hu, flags = %#hx)",
	      ptr,
	      disk_cache_info[index].ref_count,
...
create_disk_pager (void)
{
  ...
  if (!upi)
    ext4_panic ("can't create disk pager: %s", strerror (errno));
  upi->type = DISK;
  ...
  if (err)
    ext4_panic ("can't create libpager worker threads: %s", strerror (err));
}

error_t
inhibit_ext4_pager (void)
{
  error_t err;
